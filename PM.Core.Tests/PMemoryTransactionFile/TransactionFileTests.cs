...
namespace PM.Core.Tests.PMemoryTransactionFile
{
    public class TransactionFileTests : UnitTest
    {
        ...
        {
            var originalFilename = nameof(OnAddNewBlockLayout_ShouldWriteOnTransactionFile) + "originalFile";
            ...
            var transactionFile = new TransactionFile(transactionFilePm, new PAllocator(new PmCSharpDefinedTypes(CreatePmStream(originalFilename, 4096))));
            ...
            // 
            // CommitByte=01
            // Version=0001
            // =================AddBlockLayout=================
            // CommitByte=01
            ...
            Assert.StartsWith("010100010100ffffffff08000400000", ByteArrayToHexStringConverter.ByteArrayToString(content));

            transactionFile.ApplyPendingTransaction();
            ...
            // 
            // CommitByte=01
            // Version=0001
            // =================AddBlockLayout=================
            // CommitByte=01
            ...
            _output.WriteLine(result);
            Assert.StartsWith("010100010200000000001000080000000", ByteArrayToHexStringConverter.ByteArrayToString(content));

            var pmemoryDecoder = new PMemoryDecoder();
            _output.WriteLine(PMemoryDecoder.DecodeHex(ReadFileContent(CreateFilePath(originalFilename))));
        }

        [Fact]
        public void OnRemoveBlockLayout_ShouldWriteOnTransactionFile()
        {
            var originalFilename = nameof(OnRemoveBlockLayout_ShouldWriteOnTransactionFile) + "originalFile";
            ...
            var transactionFile = new TransactionFile(transactionFilePm, new PAllocator(new PmCSharpDefinedTypes(CreatePmStream(originalFilename))));
        ...
        public void UpdateContentBlockLayout_ShouldWriteOnTransactionFile()
        {
            ...
            var transactionFile = new TransactionFile(transactionFilePm, new PAllocator(null!));
        ...
        [Fact]
        public void OnApplyPendingTransaction_ShouldApply()
        {
            var originalFilename = nameof(OnApplyPendingTransaction_ShouldApply) + "originalFile";
            DeleteFile(nameof(OnApplyPendingTransaction_ShouldApply));
            DeleteFile(originalFilename);
            var transactionFilePm = CreateTransationFile(nameof(OnApplyPendingTransaction_ShouldApply));

            var pAllocator = new PAllocator(new PmCSharpDefinedTypes(CreatePmStream(originalFilename, 4096)));
            pAllocator.CreateLayout(new PersistentAllocatorLayout());
            var transactionFile = new TransactionFile(transactionFilePm, pAllocator);

            var content = Encoding.ASCII.GetBytes(@"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse accumsan feugiat euismod. In iaculis nisi vitae condimentum luctus. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Vestibulum tincidunt, lectus vel sollicitudin semper, mi lacus tempor lacus, a volutpat odio mi vitae dui. Aenean vitae tellus elit. Fusce sit amet cursus mauris. Ut convallis libero orci, quis mattis risus sodales maximus. Nullam neque lorem, consectetur in nisi eu, tincidunt pellentesque leo. Praesent ac ipsum in ante sodales finibus nec vel elit. Etiam fringilla enim pellentesque tempus lobortis. Pellentesque quis mattis tortor, sed fermentum nulla. Phasellus at euismod.");
            ...
            Assert.Equal("esperado conteudo do arquivo original", "atual");
        }
